...
<template>
	<div class="hpj mt-4 flex flex-col gap-y-4">
		...
		>
			<img src="./icon/excel-png.png" alt="" style="width: 28px;height: 28px;">
			...
			<div class="w-full flex flex-col gap-y-4">
				<div v-for="(file, index) in fileList.map(item => item.file)"
					...
					<div class="grow h-[40px] pr-6 flex items-center gap-x-3">
						<img src="./icon/excel-png.png" alt="">
					...
					<el-icon class="p-1 cursor-pointer hover:text-error"
									 @click="fileList.splice(index, 1)"><el-icon :size="14"><Delete /></el-icon></el-icon>
				</div>
			</div>
		</div>
		<div class="flex justify-end items-center">
			...
			<el-button type="primary"
								 ...
								 @click="clickOk"
			>{{!fetchUploadFile.isFetching ? '确认上传' : '上传中'}}</el-button>
		</div>
	</div>
</template>
